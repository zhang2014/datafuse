...
impl Block {
    ...
    /// Returns mask as two NEON vectors (lo, hi) for efficient SIMD operations.
    #[cfg(target_arch = "aarch64")]
    #[inline(always)]
    unsafe fn mask_neon(x: u32) -> (uint32x4_t, uint32x4_t) {
        unsafe {
            ...
            // Load all 8 salt values at once
            let salt = vld1q_u32_x2(SALT.0.as_ptr());

            // Multiply-shift hashing: (hash * salt) >> 27
            let bit_index_lo =
            ...
            // Shift 1 to the bit position: 1 << bit_index
    ...
    ///
    /// Uses vbicq_u32 (bit clear) to compute (mask & !block), then checks if result is all zeros.
    #[cfg(target_arch = "aarch64")]
    #[inline]
    fn check(&self, hash: u32) -> bool {
        unsafe {
            ...
            // vbicq_u32(a, b) = a & !b, miss = mask & !data
